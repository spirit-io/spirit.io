...
describe('*** Spirit.io REST Express routes Tests ***', () => {
    ...
    it('query with invalid where filter should throw an error', () => {
        const resp = Fixtures.get('/api/v1/myModel?where=badJson');
    ...
    it('query should return empty array', () => {
        const resp = Fixtures.get('/api/v1/myModel');
        const body = JSON.parse(resp.body);
    ...
    it('read should return not found', () => {
        const resp = Fixtures.get('/api/v1/myModel/1234');
        expect(resp.status).to.equal(404);
    });

    it('create simple instance should work', () => {
        const resp = Fixtures.post('/api/v1/myModelRel', { p1: 'prop1', pInvisible1: 'invisble1', pInvisible2: 'invisible2' });
        const body = JSON.parse(resp.body);
    ...
    it('create simple instance with invisible fields should work and return values according to conditions', () => {
        // create 2 more
        const resp = Fixtures.post('/api/v1/myModelRel', { p1: 'prop2', pInvisible1: 'invisble1', pInvisible2: 'invisible2' });
        const body = JSON.parse(resp.body);
        expect(resp.status).to.equal(201);
    ...
    it('create simple instance with readonly field modified should ignore it and raise a warning diagnose', () => {
        // create 1 more
        const resp = Fixtures.post('/api/v1/myModelRel', { p1: 'prop4', readOnlyProp: 'testModifyReadOnlyVal' });
        const body = JSON.parse(resp.body);
        ...
        expect(body.$diagnoses[0].$message).to.be
            .equal(`Property 'readOnlyProp' is readOnly and can't be modified. New value ignored: 'testModifyReadOnlyVal'; Old value kept: 'readOnlyVal'`);
        expect(body.$diagnoses[0].$stack).to.be.undefined;
    });

    const myModelRels: any[] = [];

    it('query should return the four created elements', () => {
        const resp = Fixtures.get('/api/v1/myModelRel');
        const body = JSON.parse(resp.body);
        expect(resp.status).to.equal(200);
        expect(body).to.be.a('array');
    ...
    it('not expected property should raise an error on creation', () => {
        const resp = Fixtures.post('/api/v1/myModelRel', { p1: 'prop1', p2: 'prop2' });
        const body = JSON.parse(resp.body);
        expect(resp.status).to.equal(500);
    ...
    it('create complex instance should work and return correct values', () => {
        ...
        const resp = Fixtures.post('/api/v1/myModel', data);
        expect(resp.status).to.equal(201);
    ...
    it('update complex instance with all values should work and return correct values', () => {
        ...
        const resp = Fixtures.put(`/api/v1/myModel/${myModel[0]}`, data);
        expect(resp.status).to.equal(200);
        const body = JSON.parse(resp.body);
    ...
    it('read updated instance should return correct values', () => {
        const resp = Fixtures.get(`/api/v1/myModel/${myModel[0]}`);
        expect(resp.status).to.equal(200);
        const body = JSON.parse(resp.body);
        checkComplexInstance(body, 's0updated');
    });
    ...
    it('patch complex instance with only one property should work and return correct values', () => {
        const resp = Fixtures.patch(`/api/v1/myModel/${myModel[0]}`, { pString: 's0patched' });
        expect(resp.status).to.equal(200);
        const body = JSON.parse(resp.body);
    ...
    it('read singular reference should work and return correct values', () => {
        const resp = Fixtures.get(`/api/v1/myModel/${myModel[0]}/inv`);
        expect(resp.status).to.equal(200);
        const body = JSON.parse(resp.body);
        const ref = JSON.parse(Fixtures.get(`/api/v1/myModelRel/${myModelRels[0]}`).body);
    ...
    it('query with includes should return expected elements and references', () => {
        ...
        const body2 = JSON.parse(resp.body);
        expect(resp.status).to.equal(200);
        expect(body).to.be.a('array');
    ...
    it('update complex instance with only one property should work and return only provided values', () => {
        const resp = Fixtures.put(`/api/v1/myModel/${myModel[0]}`, { pString: 's0updatedAgain', pNumber: 0, aString: ['a'] });
        expect(resp.status).to.equal(200);
        const body = JSON.parse(resp.body);
        ...
        expect(body.invs).to.equal(undefined);
    ...
    it('execute instance method that throw an exception should return diagnoses', () => {
        const resp = Fixtures.post(`/api/v1/myModel/${myModel[0]}/$execute/aMethodThatThrow`, {});
        expect(resp.status).to.equal(500);
        const body = JSON.parse(resp.body);
        expect(body.$diagnoses).to.be.not.null;
        expect(body.$diagnoses.length).to.be.equal(1);
    ...
    it('execute instance method that does not exist should return 404', () => {
        const resp = Fixtures.post(`/api/v1/myModel/${myModel[0].id}/$execute/aMethodThatDoesNotExists`, {});
        expect(resp.status).to.equal(404);
    });

    it('execute model service should work and return expected value', () => {
        const resp = Fixtures.post('/api/v1/myModel/$service/aService', { a: 2.22, b: 3.33 });
        expect(resp.status).to.equal(200);
        const body = JSON.parse(resp.body);
    ...
    it('execute model service that throw an exception should return diagnoses', () => {
        const resp = Fixtures.post('/api/v1/myModel/$service/aServiceThatThrow', {});
        expect(resp.status).to.equal(500);
        const body = JSON.parse(resp.body);
        expect(body.$diagnoses).to.be.not.null;
        expect(body.$diagnoses.length).to.be.equal(1);
    ...
    it('execute model service that does not exist should return 404', () => {
        const resp = Fixtures.post('/api/v1/myModel/$service/aServiceThatDoesNotExists', {});
        expect(resp.status).to.equal(404);
    });

    it('deleting non exiting document should return not found', () => {
        const resp = Fixtures.delete('/api/v1/myModelRel/1');
        expect(resp.status).to.equal(404);
    });

    it('query should return nothing after deleting all elements', () => {
        myModelRels.forEach((r) => {
            const _resp = Fixtures.delete(`/api/v1/myModelRel/${r}`);
            expect(_resp.status).to.equal(204);
        });
        ...
        myModel.forEach((m) => {
            const _resp = Fixtures.delete(`/api/v1/myModel/${m}`);
            expect(_resp.status).to.equal(204);
        });
    ...
    it('get request with querystring parameters on non persistent model\'s route', () => {
        const resp = Fixtures.get('/api/v1/myNotPersistentModel/sumWithQueryStringParams?a=2.22&b=3.33');
    ...
    it('post request with body parameters on non persistent model\'s route', () => {
        const resp = Fixtures.post('/api/v1/myNotPersistentModel/sumWithBodyParams', {
            a: 2.22,
            b: 3.33,
        });
        expect(resp.status).to.be.equal(200);
