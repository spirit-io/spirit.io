...
describe('*** Spirit.io ORM Framework Tests ***', () => {
    ...
    it('Instanciate class should work either with adminHelper or ModelBase methods', () => {
        ...
        const mRel1: MyModelRel = new MyModelRel({ p1: 'prop1' });
        mRel1.save();
        expect(mRel1.p1).to.equal('prop1');
        const mRel2: MyModelRel = new MyModelRel({ p1: 'prop2' });
        mRel2.save();
        ...
        const db = AdminHelper.model(MyModel);
        const m1: MyModel = new MyModel();
    ...
    it('Enums should be handled and serialized correctly', () => {
        const mWithEnum1: MyModelRel = new MyModelRel({ p1: 'propWithEnum1', pEnum: 'A' });
        mWithEnum1.save();
        expect(mWithEnum1.pEnum).to.be.equal(0);
        expect(mWithEnum1.serialize().pEnum).to.be.equal('A');
        mWithEnum1.deleteSelf();

        const mWithEnum2: MyModelRel = new MyModelRel({ p1: 'propWithEnum2', pEnum: 2 });
        mWithEnum2.save();
        expect(mWithEnum2.pEnum).to.be.equal(2);
        expect(mWithEnum2.serialize().pEnum).to.be.equal('C');
        mWithEnum2.deleteSelf();

        expect(() => new MyModelRel({ p1: 'propWithEnum3', pEnum: 'W' }).save()).to
            .throw("Invalid value for property 'pEnum'. It should be a value from 'TestEnum' enum.");
        expect(() => new MyModelRel({ p1: 'propWithEnum3', pEnum: 5 }).save()).to
            .throw("Invalid value for property 'pEnum'. It should be a value from 'TestEnum' enum.");
    });
    ...
    it('Fetch instances should allow to get relations', () => {
        const db = AdminHelper.model(MyModel);
        const rels: MyModel[] = db.fetchInstances();
    ...
    it('Update instance should allow to know if a property is modified', () => {
        const db = AdminHelper.model(MyModel);
        const rel: MyModel = db.fetchInstance(relId);
    ...
    it('Add instance\'s diagnose should allow to retrieve it on serialization', () => {
        ...
        const serialized = rel.save(null, null, {});
    ...
    it('Fetch instances should return correct results even after deleting some instances', () => {
        const db = AdminHelper.model(MyModelRel);
        const rels = db.fetchInstances();
        expect(rels.length).to.equal(3);

        const rel0 = db.fetchInstance('1234');
        expect(rel0).to.be.null;

        const rel1 = db.fetchInstance(rels[0]._id, {});
        ...
        const rel3 = db.fetchInstance(rels[2]._id, {});
